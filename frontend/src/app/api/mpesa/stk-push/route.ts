...
export async function POST(req: NextRequest) {
  try {
    const { amount, phone, userId: publicKey } = await req.json();

    console.log("Received phone number:", phone);

    if (!amount || !phone || !publicKey) {
      return NextResponse.json(
        { error: "Amount, phone, and userId are required" },
        { status: 400 }
      );
    }
    ...
    const payload = {
      ...
      AccountReference: `Zyra Deposit ${publicKey}`,
